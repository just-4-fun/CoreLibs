...
import just4fun.android.core.app.{ActivityModule, AndroidModule}
...
trait AsyncVar[T] extends Var[Async[T]] {
	def apply: Async[T]
	def update(v: T)
	def releaseValue()
}
...
object TempVar {
	...
	/** WARN: sync refactoring with macros */
	def apply[T: Manifest](name: String, value: => T)(implicit context: ActivityModule[_], typ: PropType[T]): TempVar[T] = {
...
/*FILE*/
object FileVar {
	implicit def var2future[T](v: FileVar[T]): Async[T] = v.apply
	def apply[T](filename: String): FileVar[T] = macro MacroDefs.genFileVar[T]
	/** WARN: sync refactoring with macros */
	def apply[T: Manifest](name: String, fileName: => String)(implicit context: AndroidModule, typ: ReachSchema[T]): FileVar[T] = {
...
final class FileVar[T] private(val name: String, fileName: String)(implicit context: AndroidModule, typ: ReachSchema[T]) extends AsyncVar[T] {
	private[this] implicit val futureContext = ThreadPoolContext
	private[this] var value: Async[T] = null.asInstanceOf[Async[T]]
	private[this] var inited = false
	def apply(): Async[T] = {
	...
	def releaseValue(): Unit = {
		inited = false
		value = null.asInstanceOf[Async[T]]
	}
	private def load(): Async[T] = Async {
		val bytes: Array[Byte] = Array('a', 'b', 'c')
		// TODO typ.setValuesBytes(bytes)
		null.asInstanceOf[T]
	}
	private def save(v: T): Async[T] = Async {
