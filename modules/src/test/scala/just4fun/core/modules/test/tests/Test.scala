...
//	val auto = true
	...
	if (auto) {
		autoTest(t42, t43)
//		autoTest(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, t18, t19, t20, t21, t22, t23, t24, t25, t26, t27, t28, t29, t30, t31, t32, t33, t34, t35, t36, t37, t38, t39, t40, t41, t42, t43)
		// 200 sec
	}
	else manualTest {
//		cfg1.setInject(ModCreate, true, p ⇒ m1.internal.internalBind(classOf[M2], true))
	} {
		case (s1, "ns", s2) ⇒ val s = new TestSystem; s.start(classOf[M1], 0, () ⇒ new M1); s.stop(classOf[M1])
		case (s1, "nc", s2) ⇒ val s = new TestSystem; val c = s.moduleConnector(classOf[M1], () ⇒ new M1); c.module.use(); c.moduleDisconnect()
		case (s1, "ne", s2) ⇒ val m = new EM1(1); try new EM1(2) catch loggedE; m.use(); m.stopSelf()
		case (s1, "nex", s2) ⇒ val m = new M1; m.use().onComplete {res ⇒  logV(s"[M1] use complete $res") }
	...
	/*TESTS*/
	lazy val t43 = new AutoTest("Bind self - Unbind self", "1s 1b2 2ss 1sx ///5000 2ssx") {
		override def assertions: Seq[Assertion] = Seq(
			ModDestroy(1)>>>>SysModDestroy()>@2000>>ModBindRemove(2)>>>>ModDestroy(2)>?
		)
	}
	...
	lazy val t32 = new AutoTest("Fail SyncServer in Constructor - Try Recover SyncServer", "2f0 1s 2fx 1sx") {
		cfg1.setInject(ModCreate, true, p ⇒ m1.internal.bind(classOf[M2], true))
		override def assertions: Seq[Assertion] = Seq(
	...
	lazy val t14 = new AutoTest("No delays - Start Restful - Use - Suspend -Use - Release - Stop", "1#dd0 1#ad0 1#rl0 1#dl0 1#sr1 1s 1u 1p 1u 1px 1sx") {
		override def assertions: Seq[Assertion] = Seq(
			ModReqAdd(1) >> ModActStart(1, true) >>>> ModActCompl(1, true) >> ModDeactStart(1, false) >> ModDeactProgress(1, false) >>>> ModDeactProgress(1, true) >>>> ModDeactCompl(1, false) >>>> ModActStart(1, false) >>>> ModActCompl(1, false) >> ModReqExec(1, 1) >> ModReqRemove(1) >>>> ModReqExec(1, 2) >>>> ModReqRemove(1) >> ModDeactStart(1, true) >> ModDeactProgress(1, false)  >>>> ModDeactProgress(1, true) >> ModDeactCompl(1, true) >? "1",
...
abstract class TestExternalModule(n: Int) extends TestModule {
	override def system: TestSystem = Test.system
}
class EM1(n: Int) extends TestExternalModule(n) {
	Thread.sleep(1000)
	bind[M1]
}
